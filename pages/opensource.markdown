...
description: A searchable list of open-source LLM tools
---

<h2 style="font-size: 1.8em; margin-bottom: 10px;">🧰 Open Source LLM Tools</h2>
...
<table id="tools-table" class="display stripe hover" style="width:100%; display:none;">
  <thead>
    <tr>
      <th>RepoHTML</th>   <!-- 0: clickable owner/repo HTML -->
      <th>CategoryHTML</th><!-- 1: category chip HTML -->
      <th>Stars</th>      <!-- 2: numeric stars -->
      <th>DescHTML</th>   <!-- 3: truncated description HTML -->
      <th>cat_raw</th>    <!-- 4: plain category for robust filtering -->
      <th>repo_raw</th>   <!-- 5: owner/repo plain -->
      <th>url</th>        <!-- 6: repo URL -->
      <th>raw</th>        <!-- 7: aggregate searchable text -->
    </tr>
  </thead>
  <tbody></tbody>
</table>
...
(function(){
  ...
  function renderCards(){
    ...
    rows.forEach(r => {
      ...
      // Ensure clickable title if not already wrapped
      if (url){
  ...
  async function start(){
    try{
      ...
      const rows = data.map(tool => {
        ...
        return [
          titleHTML,       // 0 RepoHTML
          catChip,         // 1 CategoryHTML
          starsNum,        // 2 Stars (num)
          descHTML,        // 3 DescHTML
          category,        // 4 cat_raw
          repo,            // 5 repo_raw
          url,             // 6 url
